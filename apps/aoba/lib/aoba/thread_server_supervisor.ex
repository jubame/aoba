defmodule Aoba.ThreadServerSupervisor do
  ...
  ** (ArgumentError) supervisors expect each child to be one of the following:
  ...
  def get_children_gt(start_thread_id) do
    ...
    Enum.reduce(
      ...
      fn child_info, acc ->
        ...
        accumulate_children(acc, child_pid, start_thread_id)
      end
    )
  end

  defp accumulate_children(acc, child_pid, _start_thread_id) when child_pid == :restarting do
    acc
  end

  defp accumulate_children(acc, child_pid, start_thread_id) do
    {:ok, ids} = Aoba.ThreadServer.get_ids(child_pid)
    if ids.thread_id > start_thread_id do
      [
